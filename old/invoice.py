...
for row in ws.iter_rows(min_row=2, values_only=True):
    customer_name = row[customer_name_index - 1] or ""
    customer_phone = row[customer_phone_index - 1] if row[customer_phone_index - 1] is not None else ""
    ...
    if response.status_code == 401:
        ...
        for row_number, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
            customer_name = row[customer_name_index - 1] or ""
            customer_phone = str(row[customer_phone_index - 1]) if row[customer_phone_index - 1] is not None else ""
            ...
            with open(f"{formname}.txt", "r") as f:
                for line in f:
                    if customer_name in line and customer_phone in line:
                        customer_number = line.split("Customer number: ")[1].strip()
                        ws.cell(row=row_number, column=customer_number_index).value = customer_number
                        break
                else:
